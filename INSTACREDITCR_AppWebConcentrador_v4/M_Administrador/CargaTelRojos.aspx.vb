...
Partial Class CargaTelRojos
    ...
    Dim Ruta As String = StrRuta() & "NumRojos\"
        ...
        Try
            Try
                ...
                ElseIf DdlTipo.SelectedValue = "Seleccione" Then
                    aviso("Seleccione El Tipo De Carga"
                   )
                ...
                Else
                    ...
                    For Each singleFile In allFiles
                        ...
                        Else
                            Dim TABLA As String = "TMP_BLACK_LIST_TEL"
                            ctlCarga = Ruta & "CTL_BLT_" & Now.ToShortDateString.Replace("/", "") & "" & ".ctl"
                            logCarga = Ruta & "LOG_BLT_" & Now.ToShortDateString.Replace("/", "") & "" & ".log"
                            badCarga = Ruta & "C:\Users\ccasanova\Documents\clientes\TUIIO\errorescargas.csv"
                            ...
                            sw.WriteLine("(")
                            sw.WriteLine("TMP_BL_CREDITO,")
                            sw.WriteLine("TMP_BL_TELEFONO")
                            sw.WriteLine(")")
                            sw.Close()
                            fs.Close()
                            ...
                            If (num <> 0) Then
                                ...
                                End If
                            Else
                                Dim SSCommand2 As New SqlCommand("SP_CARGAR_BLACK_LIST_TEL")
                                SSCommand2.CommandType = CommandType.StoredProcedure
                                SSCommand2.Parameters.Add("@V_Usuario", SqlDbType.NVarChar).Value = FileCarga
                                SSCommand2.Parameters.Add("@V_Tipo", SqlDbType.NVarChar).Value = badCarga
                                Dim DtsCarga As DataTable = Consulta_Procedure(SSCommand2, "SP_CARGAR_BLACK_LIST_TEL")
                                GvCargaRojos.DataSource = DtsCarga
                                GvCargaRojos.DataBind()
                                aviso("Fin De La Carga")
                            End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("Usuario") IsNot Nothing Then
            ...
            'If CType(Session("USUARIOADMIN"), USUARIO).CAT_LO_MADMINISTRADOR.ToString.Substring(20, 1) = 0 Then
            ...
            Try
                ...
                Else
            OffLine(HidenUrs.Value)
            AUDITORIA(HidenUrs.Value, "Administrador", "CargaTelRojos", " ", "Page_Load", "", Request.ServerVariables("REMOTE_ADDR"), Request.ServerVariables("REMOTE_HOST"))
            Session.Clear()
            Session.Abandon()
            Response.Redirect("~/SesionExpirada.aspx")
        End If
    ...
    Private Sub SendMail(ByRef evento As String, ByVal ex As Exception, ByVal Cuenta As String, ByVal Captura As String, ByVal usr As String)
        EnviarCorreo("Administrador", "CargaTelRojos.aspx", evento, ex, Cuenta, Captura, usr)
    End Sub
