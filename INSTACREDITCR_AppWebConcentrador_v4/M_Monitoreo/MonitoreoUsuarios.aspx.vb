...
Partial Class M_Monitoreo_MonitoreoUsuarios
    ...
    Public Shared Function ObtenerCartera(ByVal V_Agrupacion As String, ByVal V_Tipo As String, ByVal v_Filtro As String, ByVal v_Tipotope As String) As String
        ...
        If DtsMonitoreo.Rows.Count > 0 Then
            ...
            For i As Integer = 0 To DtsMonitoreo.Rows.Count - 1
                ...
                dtRow.Item("Fecha") = DtsMonitoreo.Rows(i)("FECHA")
                dtRow.Item("Asignacion") = DtsMonitoreo.Rows(i)("ASIGNACION")
                dtRow.Item("Latitud") = DtsMonitoreo.Rows(i)("LATITUD")
                dtRow.Item("Longitud") = DtsMonitoreo.Rows(i)("LONGITUD")
                dtRow.Item("Campaña") = DtsMonitoreo.Rows(i)("CAMPAÑA")
                dtRow.Item("Bloque") = DtsMonitoreo.Rows(i)("BLOQUE")
                dtRow.Item("Tipo") = DtsMonitoreo.Rows(i)("TIPO")
                dtRow.Item("Estatus") = DtsMonitoreo.Rows(i)("ESTATUS")
    ...
    Private Sub RcbCat_lo_usuario_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles RcbCat_lo_usuario.SelectedIndexChanged
        'TxtAutoejecuta.Text = 0
        If Rad_Tcadena(RcbCat_lo_usuario) <> "" Then
            'PnlBuscar.Visible = True
            RadBtnObtener.Visible = True
            RadBtnObtener.Enabled = True
            BtnRuta.Visible = True
            BtnRuta.Enabled = True
            'TRFecha.Visible = True
        Else
            RadBtnObtener.Enabled = False
        End If
        If DPRuta.SelectedDate IsNot Nothing And RcbCat_lo_usuario.CheckedItems.Count = 1 Then

            'TRSImular.Visible = False
        Else
            BtnRuta.Enabled = False
        End If
    End Sub

    Private Sub DPRuta_SelectedDateChanged(sender As Object, e As SelectedDateChangedEventArgs) Handles DPRuta.SelectedDateChanged
        If DPRuta.SelectedDate IsNot Nothing And RcbCat_lo_usuario.CheckedItems.Count = 1 Then
            'TRSImular.Visible = False
            BtnRuta.Enabled = True
        Else
    ...
    Private Sub RcbCat_lo_usuario_Init(sender As Object, e As EventArgs) Handles RcbCat_lo_usuario.Init
        If Not IsPostBack Then
            LLENAR_DROP(0, " ", sender, "V_VALOR", "T_VALOR", "Sin Usuario")
        End If
    End Sub
End Class
